...
import { AddFormState, LoadingState, ITweet } from './contracts/state';

export function* fetchTweetsRequest() {
  try {
    const items: ITweet[] = yield call(TweetsApi.fetchTweets);
...
export function* fetchAddTweetRequest({ payload }: FetchAddTweetActionInterface) {
  try {
    ...
    const item: ITweet = yield call(TweetsApi.addTweet, data);
